...
function OrganizationList() {
    ...
    useEffect(() => { loadOrganizations(); }, [accountCode]);

    const filteredOrganizations = useMemo(() => {
        const query = search.trim().toLowerCase();
        if (!query) return organizations;
        return organizations.filter(org =>
            [org.title, org.description, org.orgCode].some(value =>
    ...
    const rows = filteredOrganizations.map(org => [
        <div className="flex items-center space-x-3" key={`name-${org.orgCode}`}>
            ...
            <div className="min-w-0">
                <span className="font-medium text-white block truncate">{org.title || 'Untitled organization'}</span>
                <span className="text-xs text2 font-mono">{org.orgCode || '-'}</span>
            </div>
        </div>,
        ...
        <div className="flex flex-wrap gap-2" key={`actions-${org.orgCode}`}>
            <button onClick={() => navigate(`/home/organizations/${org.orgCode}`)} className="px-3 py-1 bg-indigo-600 hover:bg-indigo-700 rounded text-sm transition">View</button>
            <button onClick={() => navigate(`/home/organizations/${org.orgCode}/units/create`)} className="px-3 py-1 bg-green-600 hover:bg-green-700 rounded text-sm transition">Add Unit</button>
        </div>,
    ]);
        ...
        <div className="space-y-6">
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-3">
                    <button onClick={loadOrganizations} disabled={loading} className="flex items-center gap-2 px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded-lg transition disabled:opacity-50">
                        <RefreshIcon style={{ fontSize: '18px' }} /> Refresh
                    </button>
                    <button onClick={() => navigate('/home/organizations/create')} className="flex items-center gap-2 px-4 py-2 theme hover:themeHover rounded-lg transition">
                        <AddBusinessOutlinedIcon style={{ fontSize: '20px' }} /> Create Organization
                    </button>
                </div>
            </div>

            {error && <div className="bg-red-900/40 border border-red-700 text-red-200 p-4 rounded-lg">{error}</div>}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
                ...
                <ThreeElementCard bg="bg-purple-600" title="Active Units" number={totalUnits}><GroupsOutlinedIcon style={{ fontSize: '28px' }} /></ThreeElementCard>
            </div>

            <div className="bg2 rounded-lg p-4">
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search organizations..." className="w-full bg-transparent border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:border-indigo-500" />
            </div>
            ...
            {organizations.length === 0 ? (
                <div className="bg2 rounded-lg p-12 text-center">
                    ...
                    <button onClick={() => navigate('/home/organizations/create')} className="inline-flex items-center gap-2 px-6 py-3 theme hover:themeHover rounded-lg transition">
                        <AddBusinessOutlinedIcon style={{ fontSize: '20px' }} /> Create Your First Organization
                    </button>
                </div>
            ) : filteredOrganizations.length === 0 ? (
                <div className="bg2 rounded-lg p-12 text-center text2">No organizations match your search.</div>
            ) : <Table title="Organizations Overview" headers={headers} rows={rows} />}
        </div>
    );
}
