...
function getAccountName(employee) {
    ...
    return account.accountName || account.accountID || account.email || account.accountCode || 'Unknown user';
}
...
function CreateTask() {
    ...
    const [form, setForm] = useState({
        title: '', description: '', organizationCode: activeRole?.orgCode || '', unitCode: '',
        responsibleCode: '', relation: 'new', relatedTaskCode: '', startTime: '', endTime: '',
        deadline: '', workMinutes: '', priority: '',
    });
    ...
    useEffect(() => {
        if (activeRole?.orgCode && !form.organizationCode) update('organizationCode', activeRole.orgCode);
    }, [activeRole?.orgCode, form.organizationCode]);
    ...
    const canSubmit = Boolean(form.title.trim() && form.organizationCode && form.unitCode && form.responsibleCode && !submitting);
    ...
    async function createTask(event) {
        ...
        setSuccess(false);
        if (!canSubmit) {
            setError('Please complete the task title, organization, unit, and responsible employee.');
            return;
        }
        if (form.relation === 'related' && !form.relatedTaskCode.trim()) {
            setError('Enter the previous task code when attaching this task to an existing task.');
            return;
        }
        setSubmitting(true);
        try {
    ...
    return (
        <div className="w-full">
            ...
            <p className="text2">The task owner is automatically you. New tasks start in the backend's <strong>created</strong> state.</p>
            <form onSubmit={createTask} className="rounded-lg bg2 p-5 mt-5 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="rounded-lg border border-indigo-500/30 bg-indigo-500/5 p-4">
                        ...
                        <p className="text2 text-sm mt-1">{loggedUser?.userInfo?.accountName || loggedUser?.userInfo?.accountID || 'Current account'} — automatically assigned to you.</p>
                    </div>

                    <div className="md:col-span-2 rounded-lg border border-gray-700 p-4">
                        <p className="text-sm font-semibold mb-3">Task relation</p>
                        <div className="flex flex-wrap gap-4">
                            <label className="flex items-center gap-2 text-sm"><input type="radio" name="relation" checked={form.relation === 'new'} onChange={() => update('relation', 'new')} /> New task</label>
                            <label className="flex items-center gap-2 text-sm"><input type="radio" name="relation" checked={form.relation === 'related'} onChange={() => update('relation', 'related')} /> Attach to a previous task</label>
                        </div>
                        {form.relation === 'related' && <div className="mt-4"><Field id="related-task" label="Previous Task Code" value={form.relatedTaskCode} onChange={value => update('relatedTaskCode', value)} placeholder="e.g. Task_xxx" required /></div>}
                        <p className="text2 text-xs mt-3">The current backend represents task hierarchy through <code>taskPath</code>, so the previous task code is used as the parent path.</p>
                    </div>
                ...
                {form.unitCode && !employeesLoading && employeeOptions.length === 0 && <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-3 text-sm text-amber-200">No employees are available in this unit.</div>}
